...
class LunarTime:
    ...
    def getNineStar(self):
        ...
        asc = False
        dong_zhi = jie_qi["冬至"].toYmd() if hasattr(jie_qi["冬至"], "toYmd") else str(jie_qi["冬至"])
        xia_zhi = jie_qi["夏至"].toYmd() if hasattr(jie_qi["夏至"], "toYmd") else str(jie_qi["夏至"])
        if dong_zhi <= solar_ymd < xia_zhi:
            asc = True
