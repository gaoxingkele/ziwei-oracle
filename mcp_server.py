...
- 奇门/遁甲/八门九星（仅占具体事件/方位/择时，不论命）→ qimen
- 六壬/四课三传 → liuren
- 周易/大衍/筮法 → iching
...
### 命理（论命）vs 占事（卜筮）—— 别把"算命"派给占事工具
- 论命组（问一生格局/性格/命运/运势/大运流年，即"人的命"）→ 只用 ziwei / bazi / astrology（+ 对应 *_period）
- 占事组（问某件具体事的吉凶成败/能不能成/哪个方位、什么时候有利）→ liuyao_verdict / meihua / liuren / iching / qimen
- 奇门遁甲(qimen) 属占事组，本项目用时家奇门（按问事当下时间起局），擅长事占+方位/择时，不做推命
  - ❌ "用奇门算算我的命/这辈子命运如何" → 不要走 qimen，改 ziwei/bazi；用户坚持要奇门时先说明奇门是占具体事的，论命请用八字/紫微
  - ✅ "奇门看我这次跳槽/求财/官司能不能成"、"往哪个方位谈生意有利" → 走 qimen
